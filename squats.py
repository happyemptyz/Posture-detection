...
class Application(tk.Tk):
    ...
    def create_widgets(self):
        button_width = 20
        button_height = 2
        tk.Button(self, text="直接進行深蹲判斷", command=self.start_pose_detection, width=button_width, height=button_height).pack(pady=30) 
...
class PoseDetection(): 
    ...
    def draw_text(self,frame,msg,width=8,font=cv2.FONT_HERSHEY_SIMPLEX,pos=(0, 0),font_scale=1,font_thickness=2,text_color=(255, 255, 0),text_color_bg=(0, 0, 0),box_offset=(20, 10)):
        offset = box_offset
        x, y = pos
        ...
        cv2.putText(frame,msg,(int(rec_start[0] + 6), int(y + text_h + font_scale - 1)),font,font_scale,text_color,font_thickness,cv2.LINE_AA)
        return text_size
    ...
    def find_angle(self, p1, p2, ref_pt=np.array([0, 0])):
        ...
        logging.info(f"Angle between points: {int(degree)} degrees")
        return int(degree)
    ...
    def _show_feedback(self, frame, c_frame, dict_maps, lower_hips_disp):
        if lower_hips_disp:self.draw_text(frame,'LOWER YOUR HIPS',pos=(30, 80),text_color=(255, 255, 255),font_scale=0.6,text_color_bg=(0, 0, 255))
        for idx in np.where(c_frame)[0]:
            self.draw_text(frame,dict_maps[idx][0],pos=(30, dict_maps[idx][1]),text_color=(255, 255, 230),font_scale=0.6,text_color_bg=dict_maps[idx][2])
        return frame
    ...
    def process(self, frame):
        ...
        if self.keypoints.pose_landmarks:
            ...
            if offset_angle > self.thresholds['OFFSET_THRESH']:
                ...
                self.draw_text(frame,"Correct: "   + str(self.state_tracker['SQUAT_COUNT']),   pos=(int(frame_width*0.68), 30),text_color=(255, 255, 255),font_scale=0.7,text_color_bg=(18, 185, 0) )
                self.draw_text(frame,"Incorrect: " + str(self.state_tracker['IMPROPER_SQUAT']),pos=(int(frame_width*0.68), 80),text_color=(255, 255, 255),font_scale=0.7,text_color_bg=( 0, 0, 255), )
                self.draw_text(frame,'Camera not aligned properly!!!',  pos=(30, frame_height-60),text_color=(255, 255, 255),font_scale=0.7,text_color_bg=(190, 190, 190), )
                self.draw_text(frame,'OFFSET ANGLE: '+str(offset_angle),pos=(30, frame_height-30),text_color=(255, 255, 255),font_scale=0.7,text_color_bg=(190, 190, 190),)
                ...
                hip_vertical_angle = self.find_angle(shldr_coord, np.array([hip_coord[0], 0]), hip_coord) # Verical Angle calculation
                ...
                self.draw_dotted_line(frame, hip_coord, start=hip_coord[1]-80, end=hip_coord[1]+20, line_color=self.COLORS['blue'])
                ...
                self.draw_dotted_line(frame, knee_coord, start=knee_coord[1]-50, end=knee_coord[1]+20, line_color=self.COLORS['blue'])
                ...
                self.draw_dotted_line(frame, ankle_coord, start=ankle_coord[1]-50, end=ankle_coord[1]+20, line_color=self.COLORS['blue'])
                
                cv2.line(frame, shldr_coord, elbow_coord,self.COLORS['light_blue'], 4, lineType=self.linetype)# Join landmarks.
                ...
                if current_state == 's1':
                    if len(self.state_tracker['state_seq']) == 3 and not self.state_tracker['Incorrect_POSTURE']:
                        ...
                        play_sound = str(self.state_tracker['SQUAT_COUNT'])      # COMPUTE COUNTERS
                    ...
                    if hip_vertical_angle > self.thresholds['HIP_THRESH'][1]:
                        self.state_tracker['DISPLAY_TEXT'][0] = True           #PERFORM FEEDBACK ACTIONS
                    ...
                    if (ankle_vertical_angle > self.thresholds['ANKLE_THRESH']):
                        ...
                        self.state_tracker['Incorrect_POSTURE'] = True
                display_inactivity = False        #COMPUTE INACTIVITY 
            ...
            self.state_tracker['start_inactive_time'] = end_time
            self.draw_text(frame,"Correct: " + str(self.state_tracker['SQUAT_COUNT']),pos=(int(frame_width*0.68), 30),text_color=(255, 255, 255),font_scale=0.7,text_color_bg=(18, 185, 0))
            self.draw_text(frame,"Incorrect: " + str(self.state_tracker['IMPROPER_SQUAT']),pos=(int(frame_width*0.68), 80),text_color=(255, 255, 230),font_scale=0.7,text_color_bg=(0, 0, 255),)
            ...
            self.state_tracker['prev_state'] = None  #Reset all other state variables
            self.state_tracker['curr_state'] = None
            self.state_tracker['INACTIVE_TIME_FRONT'] = 0.0
            self.state_tracker['Incorrect_POSTURE'] = False
...
def main(detector):  
    ...
    out = cv2.VideoWriter('test1.mp4', fourcc, 10.0, (640, 480))
    pTime = 0 
    while True:
